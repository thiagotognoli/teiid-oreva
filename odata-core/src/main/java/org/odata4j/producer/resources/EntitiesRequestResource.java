...
public class EntitiesRequestResource extends BaseResource {
  ...
  public Response createEntity(
    ...
    if (producer.getMetadata().containsEdmFunctionImport(entitySetName)) {
      ...
      return FunctionResource.callFunction(callingMethod, httpHeaders, uriInfo, securityContext, producer,
          entitySetName, format, callback, query, null, null, payload);
    }
     ...
     * If slug header is present then we will treat the POST request as a MLE
     * request.
     */
    String slugHeader = httpHeaders.getRequestHeader("Slug") != null ? httpHeaders.getRequestHeader("Slug").get(0)
        : null;
  ...
  protected Response createEntity(
    ...
    if (mediaTypeList != null) {
      ...
      writer = FormatWriterFactory.getFormatWriter(EntityResponse.class, httpHeaders.getAcceptableMediaTypes(), null,
          null);
    }
  ...
  protected Response getEntitiesImpl(
    ...
    if (isCount) {
      ...
      // Use the buffered response writer for setting the response using the charset
