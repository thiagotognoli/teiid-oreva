...
public abstract class BaseResource {
  ...
  protected static OEntity convertFromString(String requestEntity, MediaType type, ODataVersion version,
      EdmDataServices metadata, String entitySetName, OEntityKey entityKey, Boolean isResponse)
      throws NotAcceptableException {
    // previously we are hard coding it to have false, since we always get the
    // entity to be created as name, value pair.
    // setting the isResponse to true only when we are building the OEntity from the
    // response as it contain root element, metadata, data,etc
